...
#include "delay_timer.h"  // this is BC only because I think this is how it should work
  /* This extern bool stuff is how I did it so it works. Profezzorn's suggestion to "change PropBase::chdir..... 
  Just move the everything inside the first "ifdef ENDBLE_AUDIO" block down into the beginning of the second "ifdef ENABLE_AUDIO" block. 
  That way Deactivate/Activate happens right next to each other, and errors should work.
Doesn't solve the overlap problem though, we might need SoundQueue for that."                doesn't work */
...
// custom_beep 0.5,523.26;0.5,587.32;0.5,523.26;0.5,392.0;1.0,261.62;
...
#ifdef ENABLE_AUDIO
PVLOG_NORMAL << "************* Font dir not found sound length = " << SaberBase::sound_length << "\n";
  if (SaberBase::sound_length > 0) return;
#ifndef DISABLE_TALKIE
  talkie.Say(talkie_font_directory_15, 15);
  talkie.Say(talkie_not_found_15, 15);
  // DodgeSound(2000);
  StartDelayTimer(2000);  // this is BC only because I think this is how it should work
#else
...
// custom_beep 0.5,523.26;0.1667,493.88;0.1667,440.0;0.1667,392.0;0.5,349.22;0.5,293.66;0.5,261.62;
...
#ifdef ENABLE_AUDIO
PVLOG_NORMAL << "************* voice_pack_not_found sound length = " << SaberBase::sound_length << "\n";
  if (SaberBase::sound_length > 0) return;
#ifndef DISABLE_TALKIE
...
#ifdef ENABLE_AUDIO
PVLOG_NORMAL << "************* error_in_blade_array sound length = " << SaberBase::sound_length << "\n";
  if (SaberBase::sound_length > 0) return;
...
// custom_beep 0.25,349.22;0.25,392.0;0.25,349.22;0.25,329.62;0.3,293.66;0.2,0;0.5,293.66;1.0,261.62;
...
#ifdef ENABLE_AUDIO
PVLOG_NORMAL << "************* error_in_font_directory sound length = " << SaberBase::sound_length << "\n";
  if (SaberBase::sound_length > 0) return;
#ifndef DISABLE_TALKIE
  talkie.Say(talkie_error_in_15, 15);
  talkie.Say(talkie_font_directory_15, 15);
...
// custom_beep 0.25,349.22;0.25,392.00;0.25,349.22;0.25,329.63;0.5,293.66;0.5,329.63;0.5,392.00;0.5,493.88;0.5,523.25
...
#ifdef ENABLE_AUDIO
PVLOG_NORMAL << "************* error_in_voice_pack_version sound length = " << SaberBase::sound_length << "\n";
  if (SaberBase::sound_length > 0) return;
#ifndef DISABLE_TALKIE
  talkie.Say(talkie_error_in_15, 15);
...
// custom_beep 0.25,349.22;0.25,392.0;0.25,349.22;0.25,329.62;0.5,440.00;0.5,293.66;1.0,392.0;0.5,261.62;
