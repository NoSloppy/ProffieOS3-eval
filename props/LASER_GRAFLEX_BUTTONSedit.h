...
SaberFett263Buttons() : PropBase() {}
...
#ifdef ON_DELAY
      STDOUT << "********************* ON_DELAY define recognized\n";
      STDOUT << "********************* ON time elapsed: " << millis() - saber_on_time_;
      // Handle Auto LASER OFF
      if (millis() - saber_on_time_ > ON_DELAY) {
        Off();
  ...
  void VolumeUp() {
    ...
    if (dynamic_mixer.get_volume() <= VOLUME) {
      dynamic_mixer.set_volume(std::min<int>(VOLUME + VOLUME * 0.02,
               dynamic_mixer.get_volume() + VOLUME * 0.08));
      beeper.Beep(0.05, 2000);
      STDOUT.print("Current Volume: ");
      STDOUT.println(dynamic_mixer.get_volume());
    }
    else {
      beeper.Beep(0.05, 1000);
    }
  }
  void VolumeDown() {
    STDOUT.println("Volume Down");
    if (dynamic_mixer.get_volume() > (0.02 * VOLUME)) {
      dynamic_mixer.set_volume(std::max<int>(VOLUME * 0.02,
               dynamic_mixer.get_volume() - VOLUME * 0.08));
      beeper.Beep(0.05, 2000);
      STDOUT.print("Current Volume: ");
      STDOUT.println(dynamic_mixer.get_volume());
...
#ifdef OFF_DELAY
        STDOUT << "********************* OFF_DELAY define recognized\n";
        if (millis() - saber_off_time_ > OFF_DELAY) {
          On();
          saber_on_time_ = millis();
        } else {
          STDOUT << "********************* OFF_DELAY not exceeded, moving on\n";
          STDOUT << "********************* OFF time elapsed: " << millis() - saber_off_time_;
          hybrid_font.PlayCommon(&SFX_bladeout);
        }
#endif
      }
        return true;
    ...
    next_preset();
      ...
      case EVENTID(BUTTON_POWER, EVENT_PRESSED, MODE_OFF):
        ...
        if (mode_volume_) {
          mode_volume_ = false;
          {
            beeper.Beep(0.1, 2800);
            beeper.Beep(0.1, 0);
            beeper.Beep(0.1, 2300);
          }
